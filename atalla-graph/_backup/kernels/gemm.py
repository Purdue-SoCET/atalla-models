...
def gemm_c(M: int, N: int, K: int) -> str:
    ...
    return (
        ...
        f"                while (wi < {tile_k}) {{\n"
        "                    int w_row = sp_w + wi;\n"
        f"                    vec wvec = vector_load(w_row, ncols, {tn1}, 1);\n"
        "                    load_weights(wvec);\n"
        "                    wi = wi + 1;\n"
        "                }\n"
        "\n"
        ...
        "                int ri = 0;\n"
        f"                while (ri < {tile_m}) {{\n"
        f"                    vec a_row = vector_load(ri, ncols, {tk1}, 0);\n"
        f"                    vec c_row = vector_load(ri, ncols, {tn1}, 1);\n"
        "\n"
        "                    vec result = gemm(a_row, c_row, all_mask);\n"
        "\n"
        f"                    vector_store(result, ri, ncols, {tn1}, 1);\n"
        "                    ri = ri + 1;\n"
        "                }\n"
        "\n"
