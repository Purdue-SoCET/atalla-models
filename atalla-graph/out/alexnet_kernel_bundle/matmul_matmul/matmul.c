int main() {
    ...
    int sp_c = 0;
    int sdma_ctl_a;
    asm("li_s %0, 32505887" : "=r"(sdma_ctl_a));
    int sdma_ctl_w;
    asm("li_s %0, 2146435111" : "=r"(sdma_ctl_w));
    int sdma_ctl_c;
    asm("li_s %0, 1106247719" : "=r"(sdma_ctl_c));
    ...
    while (mi < M_tiles) {
        ...
        while (ni < N_tiles) {
            ...
            while (ki < K_tiles) {
                ...
                while (wi < 32) {
                    int w_row = sp_w + wi;
                    vec wvec = vector_load(w_row, ncols, 31, 1);
                    load_weights(wvec);
                    wi = wi + 1;
                }
                ...
                int ri = 0;
                while (ri < 1) {
                    vec a_row = vector_load(ri, ncols, 31, 0);
                    vec c_row = vector_load(ri, ncols, 31, 1);

                    vec result = gemm(a_row, c_row, all_mask);

                    vector_store(result, ri, ncols, 31, 1);
                    ri = ri + 1;
                }
