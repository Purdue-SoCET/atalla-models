int main() {
    ...
    int sdma_ctl_w;
    asm("li_s %0, 2123366409" : "=r"(sdma_ctl_w));
    int sdma_ctl_c;
    asm("li_s %0, 1083179017" : "=r"(sdma_ctl_c));
    ...
    while (mi < M_tiles) {
        ...
        while (ni < N_tiles) {
            ...
            while (ki < K_tiles) {
                ...
                while (wi < 32) {
                    int w_row = sp_w + wi;
                    vec wvec = vector_load(w_row, ncols, 9, 1);
                    load_weights(wvec);
                    wi = wi + 1;
                }
                ...
                while (ri < 1) {
                    ...
                    vec c_row = vector_load(ri, ncols, 9, 1);

                    vec result = gemm(a_row, c_row, all_mask);

                    vector_store(result, ri, ncols, 9, 1);
                    ri = ri + 1;
                }
