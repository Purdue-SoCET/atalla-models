int main() {
    ...
    int sdma_ctl_w;
    asm("li_s %0, 1342177280" : "=r"(sdma_ctl_w));
    int sdma_ctl_c;
    asm("li_s %0, 2113929216" : "=r"(sdma_ctl_c));
    ...
    while (mi < M_tiles) {
        ...
        while (ni < N_tiles) {
            ...
            while (ki < K_tiles) {
                ...
                while (wi < 9) {
                    int w_row = sp_w + wi;
                    vec wvec = vector_load(w_row, ncols, 0, 1);
                    load_weights(wvec);
                    wi = wi + 1;
                }
                ...
                while (ri < 32) {
                    ...
                    vec c_row = vector_load(ri, ncols, 0, 1);

                    vec result = gemm(a_row, c_row, all_mask);

                    vector_store(result, ri, ncols, 0, 1);
                    ri = ri + 1;
                }
